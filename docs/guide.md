...
- **Observations are never rails.** `possible-loop`, `BUDGET-EXCEEDED`,
  ...
  substrate refusals that protect a checkout (`runner.lock`, an attached
  task, unfinished dependencies), and none of them second-guesses a
  decision.
...
tell the manager, run, cancel, ack an escalation),
