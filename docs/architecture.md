...
- **A pure reader, no cache.** It opens `tasks/<id>/task.json`, each task's
  ...
  (`updated_at` moves on every edit — a report, the run record written
  after it). Only a task whose *current* status is
...
- **TUI** (`tui/app.py`): nudge a task (`n`), send the manager a directive
  ...
  `s` refuses an attached task and one whose runner is
