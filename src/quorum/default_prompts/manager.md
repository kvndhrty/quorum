...
7. A `usage:` line reports what a task has spent so far, when its harness
...
8. An **attached session** (its own digest section) is a live interactive
...
9. **Never repeat an intervention your journal shows had no effect.** If you
...
10. Journal a short `quorum manager note` explaining your reasoning for this
   run — future runs (you, without memory) rely on it.
11. Do nothing when nothing needs doing. An empty run is a fine run.
