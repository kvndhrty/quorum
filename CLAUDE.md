...
- `tasks.py` — the task substrate: `Task`/`TaskStore` over `tasks/<id>/task.json`,
  ...
  **loud**, not soft — see `forge.py`.
  ...
  Quorum orders nothing: `TaskStore.list` is chronological and no reader
  sorts, so what runs next is the manager's judgement from the digest, and
  `--after` is the only ordering the substrate enforces. The `priority` and
  `held` fields and the `task hold` / `task release` / `task set-priority`
  commands were removed in #102 (no use in the dogfood home); old task.json
  records still carry the keys and the model ignores them.
