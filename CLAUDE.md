...
- `runner.py` — one harness run: `runner.lock` pid-lock → git worktree under
  ...
  exits — `auto_commit_workdir`, a mechanical safety net that never pushes, never
  sets status, never fires outside a task's own worktree, and records its own
  failures in the transcript instead of raising. The runner **never sets task
